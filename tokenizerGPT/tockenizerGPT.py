...
############
testing = True
saved_model = "tokenizerGPT/AI_and_bees_model_parameters.pth"
############
...
eval_iters = 200
n_embd = 64 #384
head_size = 32 # n_embd // n_head
n_head = 4 #6
n_layer = 3 #6
# dropout drops a percent of the neurons in the previous layer, which is helpful for both speed and for testing
# prevents overfitting -> changed from 0.2 to 0.4 due to overfitting (val loss stayed around 4.5 while test loss kept going down)
dropout = 0.4 #0.2
temperature = 1

with open('tokenizerGPT/output.txt', 'r', encoding='utf-8') as f:
    text = f.read()

with open("tokenizerGPT/tokened.txt", "rb") as f:
    vocab = pickle.load(f)

with open("tokenizerGPT/merges.txt", "rb") as f:
...
#if data is saved - sometimes overridden if model is overfitting or want to change input
...
input_data = "\n"
context = torch.tensor(encode(input_data)).unsqueeze(0)
print(decode(model.generate(context, max_new_tokens=500)[0].tolist()))
open('tokenizerGPT/more.txt', 'w', encoding='utf-8').write(decode(model.generate(context, max_new_tokens=10000)[0].tolist()))